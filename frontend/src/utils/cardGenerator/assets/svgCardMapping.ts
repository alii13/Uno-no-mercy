...
/**
 * Generate all 168 card definitions
 */
export function generateAllCards(): Array<{
  id: string
  color: CardColor
  type: CardType
  value?: number
  ...
  const generateId = () => `card-${cardId++}`
  
  // Numbers 0-9: 2 of each number per color = 80 cards (2×10×4)
  colors.forEach(color => {
    // All numbers 0-9: 2 of each per color
    for (let num = 0; num <= 9; num++) {
      for (let i = 0; i < 2; i++) {
        cards.push({
          ...
          type: 'number',
          value: num,
          svg: getCardSVG('number', num, color),
        })
      }
    }
  })
  ...
  colors.forEach(color => {
    for (let i = 0; i < 3; i++) {
      cards.push({
        ...
        color,
        type: 'skip',
        svg: getCardSVG('skip', undefined, color),
      })
    }
  })
  ...
  colors.forEach(color => {
    for (let i = 0; i < 2; i++) {
      cards.push({
        ...
        color,
        type: 'skipEveryone',
        svg: getCardSVG('skipEveryone', undefined, color),
      })
    }
  })
  ...
  colors.forEach(color => {
    for (let i = 0; i < 3; i++) {
      cards.push({
        ...
        color,
        type: 'reverse',
        svg: getCardSVG('reverse', undefined, color),
      })
    }
  })
  ...
  colors.forEach(color => {
    for (let i = 0; i < 2; i++) {
      cards.push({
        ...
        color,
        type: 'draw2',
        svg: getCardSVG('draw2', undefined, color),
      })
    }
  })
  
  // Draw Four: 2 per color
  colors.forEach(color => {
    for (let i = 0; i < 2; i++) {
      cards.push({
        id: generateId(),
        color,
        type: 'draw4',
        svg: getCardSVG('draw4', undefined, color),
      })
    }
  })
  
  // Discard All: 3 per color
  colors.forEach(color => {
    for (let i = 0; i < 3; i++) {
      cards.push({
        id: generateId(),
        color,
        type: 'discardAll',
        svg: getCardSVG('discardAll', undefined, color),
      })
    }
  })
  
  // Wild Reverse Draw Four: 8 total
  for (let i = 0; i < 8; i++) {
    cards.push({
      id: generateId(),
      color: 'red' as CardColor, // Wild cards still need a color for type system
      type: 'wildReverseDraw4',
      svg: getCardSVG('wildReverseDraw4', undefined, 'wild'),
    })
  }
  
  // Wild Draw Six: 4 total
  for (let i = 0; i < 4; i++) {
    cards.push({
      id: generateId(),
      color: 'red' as CardColor,
      type: 'draw6',
      svg: getCardSVG('draw6', undefined, 'wild'),
    })
  }
  
  // Wild Draw Ten: 4 total
  for (let i = 0; i < 4; i++) {
    cards.push({
      id: generateId(),
      color: 'red' as CardColor,
      type: 'draw10',
      svg: getCardSVG('draw10', undefined, 'wild'),
    })
  }
  
  // Wild Color Roulette: 8 total
  for (let i = 0; i < 8; i++) {
    cards.push({
      id: generateId(),
      color: 'red' as CardColor,
      type: 'wildColorRoulette',
      svg: getCardSVG('wildColorRoulette', undefined, 'wild'),
    })
  }
