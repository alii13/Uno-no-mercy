...
describe('autoStartTick', () => {
    ...
    it('join cuts never push the clock below the 10s floor', () => {
        ...
        s = autoStartTick(s, { ...lobby, connectedIds: ['a', 'b', 'c'], now: NOW + 18_000 })
        ...
        t = autoStartTick(t, { ...lobby, connectedIds: ['a', 'b', 'c'], now: NOW + 22_000 })
    ...
    it('a flap through a pause still costs nothing on resume', () => {
        ...
        let s = autoStartTick(undefined, { ...lobby, connectedIds: ['a', 'b'] })
        s = autoStartTick(s, { ...lobby, connectedIds: ['a'], now: NOW + 22_000 })
        expect(s.leftMs).toBe(8_000)
        s = autoStartTick(s, { ...lobby, connectedIds: ['a', 'b'], now: NOW + 23_000 })
        expect(s.at).toBe(NOW + 23_000 + 8_000)
    })
    ...
    it('a genuine newcomer resuming a paused clock gets the readable-lobby floor', () => {
        let s = autoStartTick(undefined, { ...lobby, connectedIds: ['a', 'b'] })
        // Paused with 2s left...
        s = autoStartTick(s, { ...lobby, connectedIds: ['a'], now: NOW + 28_000 })
